...
import { applyConnectorSelectionChange } from '$lib/orchestration/connectorSelectionOrchestrator';
...
vi.mock('$lib/api/config', () => ({
  setModifiedValue: vi.fn().mockResolvedValue(true),
...
describe('SegmentView connector selectors', () => {
  it('renders segment-local connector selectors when the selected segment is affected', () => {
    ...
    const select = screen.getByLabelText('Daughter board for Connector A');
    expect(select).toBeInTheDocument();
    expect(within(select).getByRole('option', { name: 'BOD4-CP - Detector board' })).toBeInTheDocument();
    expect(screen.queryByText('Detector board')).not.toBeInTheDocument();
  });
  ...
  it('updates the currently selected governed line immediately when connector selection changes', async () => {
    ...
    render(SegmentView, {
      props: {
        onchangeConnectorSelection: (event: CustomEvent<{
          ...
          void applyConnectorSelectionChange(event.detail);
        },
      },
    });
    ...
    await waitFor(() => {
      ...
      expect(inputSelect()).toHaveValue('2');
      expect(screen.queryByRole('status', { name: /incompatible/i })).not.toBeInTheDocument();
    });
  });

  it('shows the corrected governed value before the backend modified-value write resolves', async () => {
    let resolveSetModifiedValue: ((value: boolean) => void) | null = null;
    vi.mocked(setModifiedValue).mockImplementationOnce(() => new Promise<boolean>((resolve) => {
      resolveSetModifiedValue = resolve;
    }));
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('Input Function')).toHaveValue('2');
    });

    resolveSetModifiedValue?.(true);
    await waitFor(() => {
      expect(vi.mocked(setModifiedValue)).toHaveBeenCalled();
    });
  });
});