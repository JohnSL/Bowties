...
function formatConnectorSlotOptionLabel(label: string, description: string | null | undefined): string {
      ...
      options: [
        ...
        ...slot.supportedDaughterboardIds
          .map((daughterboardId) => {
            ...
            const description = daughterboard?.description ?? null;
            const label = daughterboard?.displayName ?? daughterboardId;
            return {
              value: daughterboardId,
              label: formatConnectorSlotOptionLabel(label, description),
              description,
            };
          })
