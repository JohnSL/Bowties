...
- **Contributors** (sources of the compilation + teardown state):
  - [bowties-core/src/logic_adapter/mod.rs](../bowties-core/src/logic_adapter/mod.rs) `compile_tower_lcc(template, facility, bindings, style, allocated_range)` — **Spec 020 / S1 (new):** deepest layer for compiled templates; owns the "template rules → conditional line CDI field values" contract. Returns `CompiledLogicPlan` carrying conditional line configurations and allocation record. S1 stub; S2 real compiler. Capacity enforcement (`InsufficientCapacity`) is compiler-owned. Write surface is disjoint from `compose_bowtie_ops` (conditional-line fields vs. EventID consumer leaves).
  - [app/src-tauri/src/commands/logic_adapter.rs](../app/src-tauri/src/commands/logic_adapter.rs) `compile_logic_for_facility(facility_id, target_node_key)` IPC — **Spec 020 / S1 (new):** reads through `LayoutState.effective_*` like the compose IPC. Dispatches into `compile_tower_lcc`.
