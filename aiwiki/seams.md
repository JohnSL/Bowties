...
- **Last-modified**: 2026-07-25 (Spec 020 / S6 — `deleteFacility` now gates on `findUpstreamReferrers` at the route level: if any facility has `facilityId`'s output channels bound as its `downstream-signal` input, the `DeleteFacilityDialog` confirmation dialog appears listing affected upstream facilities before proceeding. No change to channel lifecycle ordering; the reset-drafts step precedes the existing tearDown → delete → remove-user-owned-channels sequence.)
- **Last-audited**: 2026-07-07
...
- **Contributors** (sources of the per-binding path prefix):
  - [bowties-core/src/channel_events.rs](../bowties-core/src/channel_events.rs) — `resolve_connector_input_path_prefix(tree, connector, input)` (producer-side, S2 slot lookup) + `resolve_lamp_row_path_prefix(tree, row_ordinal)` (consumer-side, walks `Direct Lamp Control/Lamp#N`) + `resolve_lamp_row_range_event_ids(tree, start_row, row_count, role, leaf_index_map)` (Spec 020 / S3 — multi-row consumer resolution; iterates N consecutive Lamp rows and concatenates their EventId leaves before indexing).
  - [app/src-tauri/src/commands/channel_events.rs](../app/src-tauri/src/commands/channel_events.rs) — `resolve_channel_event_ids` IPC command; `ChannelResolutionRequest` carries `binding` (tag-discriminated union mirroring `ChannelBinding`) + `role` + `leaf_index_map`; dispatches per `binding.kind` and returns the resolved map per channel. **Spec 020 / S3:** `LampRowRange { startRowOrdinal, rowCount }` binding variant added for multi-row signal-aspect channels.
  - [app/src/lib/orchestration/eventStateOrchestrator.ts](../app/src/lib/orchestration/eventStateOrchestrator.ts) — frontend payload builder; picks `role: 'producer'` for `block-occupancy` channels and `role: 'consumer'` for `lamp-indicator` and `signal-aspect` channels; pulls the leaf-index map from `getStyleEventMapping(channel.style)` (using `producerLeafIndex` / `consumerLeafIndex` per role). **Spec 020 / S3:** signal-aspect channels with `lampRow` binding are promoted to `lampRowRange` with `rowCount` from `getStyleRowCount(ch.style)`.
  - [app/src/lib/utils/channelStyles.ts](../app/src/lib/utils/channelStyles.ts) — `STYLE_EVENT_MAPPINGS` is the single source of truth for "style → state-name → leaf ordinal". Each entry carries either `producerLeafIndex` OR `consumerLeafIndex` (mutually exclusive per state). **Spec 020 / S3:** `2-led-bicolor-aspect` maps LED-level events (`redOn`/`redOff`/`greenOn`/`greenOff`) for state display (not compilation — compiler uses its own `AspectPinMap`).
- **Consumers** (readers of the resolved map):
  ...
  - [app/src/lib/components/Railroad/ChannelsPanel.svelte](../app/src/lib/components/Railroad/ChannelsPanel.svelte) — per-channel `channelStates` derivation pairs the resolved map with `eventStateStore` PCERs. **Spec 020 / S3:** dispatches `deriveSignalAspectState(events, redOn, redOff, greenOn, greenOff)` for `signal-aspect` channels (4-event → 2×2 LED combination → aspect), `deriveChannelState(events, positive, negative, role)` for `block-occupancy` and `lamp-indicator`.
...
- **Last-modified**: 2026-07-19 (Spec 020 / S3 — added `LampRowRange` binding + signal-aspect multi-row resolution + `deriveSignalAspectState` consumer)
- **Last-audited**: 2026-07-07
...
- **Owner**: [app/src/lib/orchestration/facilityOrchestrator.ts](../app/src/lib/orchestration/facilityOrchestrator.ts) `compileLogicIfNeeded(facilityId)` + `composeBowtiesIfWired(facilityId)` + `tearDownFacilityBowties(facilityId)` + `deleteFacility(facilityId)` wrapper. **Spec 020 / S1 — compile-before-compose:** for templates with a `compilation_target`, the orchestrator runs `compileLogicIfNeeded` (backend IPC → `logic_adapter/`) to produce CDI conditional-line drafts and record the logic allocation on the facility, then runs `composeBowtiesIfWired` for bowtie event wiring. The two phases write to disjoint CDI leaf sets and have independent teardown paths — compile teardown clears by allocation record, compose teardown clears by composer re-derivation or metadata scan. Compose is guarded by `effectiveLayoutStore.facilityStatus(facilityId) === 'Wired'`; teardown is guarded by `bowtieMetadataStore.bowtiesForFacility(facilityId)` (empty ⇒ no-op).
- **Contributors** (sources of the compilation + teardown state):
  - [bowties-core/src/logic_adapter/mod.rs](../bowties-core/src/logic_adapter/mod.rs) `compile_facility(input: &CompileInput)` — **Spec 020 / S2:** deepest layer for compiled templates; owns the "template rules → conditional line field values" contract. Returns `CompiledLogicOutput` with `Vec<UnresolvedFieldWrite>` keyed by `ConditionalLineField` enum (logical field identity, no CDI addresses). Address resolution is performed by the IPC layer via `build_conditional_line_address_map` (tree walker) + `resolve_field_writes` (join). Handles standalone (3-line) and end-of-line (2-line, Approach omitted) signals. Aspect-to-pin-action maps are compiler-owned constants. Capacity enforcement (`InsufficientCapacity`, `TooManyActions`) is compiler-owned. Write surface is disjoint from `compose_bowtie_ops` (conditional-line fields vs. EventID consumer leaves). **Spec 020 / S6 — symmetric reset:** `reset_facility(allocation: &LogicAllocation) → Vec<UnresolvedFieldWrite>` produces the inverse plan — clears every field of the allocated conditional line range to CDI defaults (Function=Blocked, VariableTrigger=Off, VariableSource=None, ActionBehavior=Off, ActionDestination=None, event IDs zeroed, mast group flags cleared, TC-write slot cleared when `track_circuit` is set). Called on facility delete so the compiler owns both forward compile and inverse reset — the frontend never derives "default" values (that is CDI-schema knowledge).
  - [app/src-tauri/src/commands/logic_adapter.rs](../app/src-tauri/src/commands/logic_adapter.rs) `compile_logic_for_facility(facility_id, target_node_key)` IPC — **Spec 020 / S2:** reads through `LayoutState.effective_*` (matching `compose_facility_bowties` pattern), resolves channel event IDs from config trees, builds `CompileInput`, calls compiler, then resolves CDI field addresses from the target node's config tree via `build_conditional_line_address_map` + `resolve_field_writes`. Returns `CompiledLogicPlan` with tree-resolved addresses. **Spec 020 / S6:** sibling command `reset_logic_for_facility(facility_id)` reads the facility's current `LogicAllocation` from effective LayoutState, calls `reset_facility()`, resolves addresses via the same helpers, returns the field writes; empty vec if no allocation (idempotent).
  - [app/src/lib/api/logicAdapter.ts](../app/src/lib/api/logicAdapter.ts) `compileLogicForFacility(facilityId, targetNodeKey)` TS wrapper — **Spec 020 / S1 (new).** **Spec 020 / S6:** adds `resetLogicForFacility(facilityId)` sibling wrapper.
  - [bowties-core/src/facility_bowties/mod.rs](../bowties-core/src/facility_bowties/mod.rs) `compose_bowtie_ops(facility, template, channels, producer_event_ids, per_node_cdi, consumer_leaf_index)` — deepest layer for bowtie composition; owns the "two bowties per Block Indicator, event IDs adopted from the producer (D6), name derived from the state mapping" contract. Returns `Vec<CompositionOp>` where each op carries the consumer leaf path, consumer leaf space/address, producer event-id bytes, bowtie name, and `createdByFacility` back-reference.
...
- **Last-modified**: 2026-07-25 (Spec 020 / S6 bugfix — compile-vs-compose inverse-path symmetry contract now executable. Forward/inverse symmetry rule: any discriminator that gates the forward composition path (today: `template.compilationTarget === 'compiled'` routing to `compileIfWired` vs. `composeFacilityBowties`) MUST be mirrored at the same seam on the inverse (teardown/reset) path. `resetComposedLeavesForFacility` now short-circuits for Compiled templates before either the composer-forward or metadata-driven-fallback branch. The composer boundary itself is defended: `bowties-core::facility_bowties::compose_bowtie_ops` and the `compose_facility_bowties` IPC both return `Ok(vec![])` for Compiled templates, encoding the disjoint-write-surface invariant (conditional-line fields vs. EventID consumer leaves) as executable contract rather than merely documented — protects any future caller (`_cascadeDetach`, `reconcileDanglingChannelRefsOnLoad`, save-time re-emitters, integration tests). Earlier same-day: Spec 020 / S6 — compiler-owned symmetric reset: `reset_facility()` + `reset_logic_for_facility` IPC + `resetLogicForFacility` TS wrapper added; `deleteFacility` orchestrator now stages reset drafts BEFORE `tearDownFacilityBowties` when a `LogicAllocation` is present, ensuring conditional-line CDI drafts revert to defaults on facility delete. Reset ordering mirrors the existing `resetComposedLeavesForFacility` pattern for bowtie consumer leaves. `findUpstreamReferrers` utility + `DeleteFacilityDialog` component added as delete-confirmation gate at the route.)
- **Last-audited**: 2026-07-07
...
- **Governing ADR(s)**: ADR-0017 (Transport Health broadcast + bounded FIFO writer)
- **Owner**: `TransportActor::writer_loop` ([lcc-rs/src/transport_actor.rs](../lcc-rs/src/transport_actor.rs)) — drives every publication on the `watch::Sender<TransportHealth>` via the shared `send_frame_with_timeout` helper. Field: `TransportActor` owns the sender through `TransportHandle::health_tx`; subscribers borrow the current state and `.changed().await` transitions.
- **Contributors** (paths that publish transitions):
  - `writer_loop` ([lcc-rs/src/transport_actor.rs](../lcc-rs/src/transport_actor.rs)) — drains the outbound mpsc, wraps every `w.send(&frame).await` in `send_frame_with_timeout(writer, frame, Some(&health_tx))`. Publishes `Wedged` on `SERIAL_SEND_TIMEOUT` / `TCP_SEND_TIMEOUT` (per `TransportWriter::send_timeout()`), `Healthy` on the first subsequent success. `send_if_modified` semantics deduplicate identical states.
  - `TransportHandle::send_direct` ([lcc-rs/src/transport_actor.rs](../lcc-rs/src/transport_actor.rs)) — D2 in the S1 slice card: the inline direct-write path calls the same `send_frame_with_timeout` helper with the same `health_tx`, so a stall on the bypass path also publishes `Wedged`. Retires alongside `send_direct` itself in S5; the helper survives (retains the `writer_loop` caller).
- **Consumers** (readers of the health state):
  - `TransportHandle::send` ([lcc-rs/src/transport_actor.rs](../lcc-rs/src/transport_actor.rs)) — the first Consumer (S1). Short-circuits with `Err(Error::TransportUnhealthy(reason))` when `*health_tx.borrow()` is `Wedged`; no frame is enqueued onto the mpsc. Fail-fast is unconditional so every current and future outbound caller inherits the guarantee.
  - `PeerSession` health-forwarder task ([lcc-rs/src/peer_session.rs](../lcc-rs/src/peer_session.rs)) — S2. Subscribes on construction via `TransportHandle::subscribe_health()` and pushes `PeerCommand::TransportWedged { reason }` into the session's own command mpsc on `Healthy → Wedged` transitions. The session's `run()` loop aborts the active exchange with `PeerError::TransportUnhealthy` and skips the outbound `TerminateDueToError` emission (D1 outcome — peer cleanup requires a live wire).
  - `PeerSessionRegistry` — **planned in S3+**. Currently spawns unconditionally; may consult the seam in a later slice to gate spawn on healthy transport, and (per ADR-0017 Fairness section) is the correct future home for any per-peer output prioritization.
  - `LiveNodeProxy` — **planned in S2–S6**. Surfaces health into the frontend node snapshot so the UI can distinguish "peer is quiet" from "wire is wedged".
  - UI connection-status surface — **planned in S2–S6**. `TransportHealth::Wedged` becomes a visible connection-state indicator, replacing today's "commands hang forever" symptom.
  - `transport::gridconnect_serial` blocking writer thread ([lcc-rs/src/transport/gridconnect_serial.rs](../lcc-rs/src/transport/gridconnect_serial.rs)) — **wire-I/O layer below the actor** (S9). For this transport `TransportWriter::send()` is an mpsc *enqueue*, so the actor's `SERIAL_SEND_TIMEOUT` bounds the enqueue; the actual `write_all`/`flush` runs on a dedicated OS thread (executor isolation) with a platform-specific per-write bound (Windows: DCB `WriteTotalTimeoutConstant` patched to 5000 ms; Unix: `tcdrain()` unbounded — relies on the actor enqueue-fill `Wedged` backstop). Not a health *publisher*; documented here so the enqueue-vs-write split is discoverable from the seam.
- **Per-slice plumbing rule**: Every path that owns a `TransportWriter` and issues an outbound frame MUST route through `send_frame_with_timeout` — never call `writer.send(&frame).await` directly. The helper is the seam's single publication site; adding a second write path that bypasses it would silently break the health contract. New consumers subscribe via `TransportHandle::subscribe_health()` (returns `Option<watch::Receiver<TransportHealth>>` — `None` only on the legacy `from_parts` bridge path); readers use `rx.borrow()` for the current state and `rx.changed().await` for transitions. Per-transport-kind timeouts land in `TransportWriter::send_timeout()` overrides, not as ad-hoc `tokio::time::timeout` wraps at the call site. Adding a per-peer fairness or prioritization scheme belongs in `PeerSessionRegistry` (S2+), not in the writer — the wire is physically single-lane and per-frame `SEND_TIMEOUT` already bounds each frame's drain time.
- **Last-modified**: 2026-07-18 (S9 — recorded `gridconnect_serial` blocking writer thread as the serial wire-I/O layer; documented the enqueue-vs-write timeout split and per-platform per-write bound per ADR-0017 §2026-07-18 extension)
- **Last-audited**: 2026-07-07

### Notes

**Why `watch` over `broadcast`** (S1 decision D1, 2026-07-07): `watch::Sender::send_if_modified` gives deduplication for free (repeat `Healthy` after `Healthy` is a no-op), the "latest value is always readable via `borrow()`" property gives late subscribers a correct initial state without a `resubscribe` dance, and there is exactly one seam owner (the writer task). The `broadcast + current_health()` shape from the earlier contract draft required a parallel atomic to give late subscribers the current value, which is a second source of truth for the same fact.

**Fairness is explicitly not a transport-layer concern** (ADR-0017 Fairness section): the single-shared-FIFO writer is deliberate because the wire is physically single-lane, per-frame `SEND_TIMEOUT` bounds each frame's contribution to head-of-line blocking, and per-peer prioritization would push shared-resource scheduling into the wrong layer. If future work needs peer-fairness (e.g., a slow peer starving a chatty one), the correct seam is a scheduling policy inside `PeerSessionRegistry` deciding *which* per-peer session gets to enqueue next — not a redesign of the writer.

**Test coverage** (`lcc-rs/src/transport_actor.rs` `#[cfg(test)] mod tests`):
- `writer_bounded_send_emits_wedged_on_stall` — writer task publishes `Wedged` within `SERIAL_SEND_TIMEOUT + slack` when the writer stalls; concurrent `subscribe_all()` doesn't deadlock.
- `writer_returns_to_healthy_after_stall_clears` — clearing the stall and pushing another frame publishes `Healthy` exactly once (`send_if_modified` dedup).
- `send_short_circuits_with_transport_unhealthy_when_wedged` — `TransportHandle::send()` returns `Err(Error::TransportUnhealthy)` in <50ms when health is `Wedged`, without enqueuing.
- `send_direct_wedges_and_returns_transport_unhealthy_on_stall` — D2 assertion: `send_direct` participates in the seam via the shared helper.

The parametric stall variant on `MockTransportWriter` (`MockTransport::stall_handle() -> Arc<AtomicBool>`) is the shared fixture; new regression tests for wedge-adjacent bugs should reuse it.

**Serial transport model + enqueue-vs-write timeout split** (S9 decision, 2026-07-18, ADR-0017 §2026-07-18 extension): `gridconnect_serial` uses blocking OS reader/writer threads (kept for **executor isolation** — a stuck serial syscall never starves a tokio worker, on every platform — *not* the earlier unproven "Windows async unreliable" claim). Because `TransportWriter::send()` is an mpsc enqueue on this transport, the actor's `SERIAL_SEND_TIMEOUT` (500 ms) bounds the **enqueue**, not the wire write; it fires only once the queue backs up behind a stuck thread. The per-write bound is platform-specific (Windows 5000 ms DCB; Unix `tcdrain()` unbounded), so the actor's enqueue-fill `Wedged` detection is the uniform cross-platform observable backstop. A known Unix gap (writer thread exits on transient `write()` `TimedOut`; `tcdrain` blocks unbounded under sustained CTS back-pressure) is deferred to a `kind/idea` follow-up, to be validated on Mac/Linux hardware. `slcan_serial` remains async (`tokio_serial::SerialStream`), so `tokio-serial` stays a dependency regardless.
...
- **Owner**: `PeerSessionRegistry` ([lcc-rs/src/peer_session_registry.rs](../lcc-rs/src/peer_session_registry.rs)) — sole spawner of `PeerSession` actors keyed by `NodeID`. Runs a background spawn-watcher task subscribed to the transport inbound broadcast that calls `PeerSession::spawn` on the three NodeID-carrying MTIs (VNI 0x19170, InitializationComplete 0x19100, AliasMapDefinition 0x10701). Repeat observations dispatch `PeerCommand::AliasChanged` to the existing session — never a re-spawn. Internal map: `tokio::sync::RwLock<HashMap<NodeID, PeerSessionHandle>>` with read-check → drop-guard → build-outside-lock → write-lock-double-check discipline (per `tokio-rwlock-self-deadlock` guardrail).
- **Contributors** (paths that trigger spawn / lifecycle events):
  - `spawn_watcher_loop` ([lcc-rs/src/peer_session_registry.rs](../lcc-rs/src/peer_session_registry.rs)) — the only production caller of `PeerSession::spawn`. Qualifies inbound broadcast frames by MTI and dispatches spawn or `AliasChanged`. `broadcast::error::RecvError::Lagged(_)` on the qualifier subscription is skipped and the loop continues (missing an observation is safe — the next VNI/AMD re-establishes state).
  - `AppState::set_connection_with_dispatcher` ([app/src-tauri/src/state.rs](../app/src-tauri/src/state.rs)) — constructs the registry when the transport comes up and publishes it to `AppState::sessions` + `NodeRegistry::set_peer_sessions`. `AppState::disconnect` calls `sessions.shutdown().await` (ADR-0016 §2026-07-14 extension) which aborts the spawn-watcher `JoinHandle` and clears the sessions map so the transport-handle refcount collapses to zero and the OS serial handle releases before reconnect.
  - `PeerSessionRegistry::shutdown` ([lcc-rs/src/peer_session_registry.rs](../lcc-rs/src/peer_session_registry.rs)) — the aggregate's explicit termination entrypoint. `.take()`s the `Mutex<Option<JoinHandle<()>>>`, calls `abort()`, awaits the aborted result, then clears the sessions map. Matches the pattern used by `TransportActor::shutdown` and `LccConnection::shutdown_responders`; completes the discipline across the three aggregates in the connection stack.
  - `PeerSessionRegistry` demux task + `route_frame` ([lcc-rs/src/peer_session_registry.rs](../lcc-rs/src/peer_session_registry.rs)) — S7 (ADR-0016 §2026-07-18 extension). Demultiplexes the retained `subscribe_all()` bus into one bounded per-peer `mpsc::Sender<InboundEvent>` (`INBOUND_CAPACITY = 256`) keyed by **source alias** (`routes: RwLock<HashMap<u16, Route>>`, `Route { tx, pending_drops }`). On channel-full it drops + increments `pending_drops` and delivers a coalesced `InboundEvent::Lagged(pending)` before the next successful frame; a broadcast-level lag signals every route. Route registration is atomic with session-entry insert (lock order **sessions → routes**); `AliasChanged` re-keys route + `entry.alias`. Construction-window = DROP (unregistered source alias discarded). This supersedes D3 for CDI: sessions no longer subscribe to the bus directly, so cross-peer fan-out amplification is structurally gone.
  - `PeerSession::handle_inbound_lag` ([lcc-rs/src/peer_session.rs](../lcc-rs/src/peer_session.rs)) — S7. On `InboundEvent::Lagged` with an active `CdiDownload`: bounded in-place recovery (reset assembler + re-issue same-cursor read via `send_next_chunk_request`, cumulative `lag_recovery_count` = `config.max_retries`), then abort through the fault-nature cleanup gate on exhaustion. SNIP/PIP retain abort-and-continue; idle is ignored.
  - `PeerSession::on_cdi_frame` + `PeerSession::send_next_chunk_request` ([lcc-rs/src/peer_session.rs](../lcc-rs/src/peer_session.rs)) — per-session CDI Contributors added in S3 (ADR-0018). Own the CDI multi-chunk state machine: MemoryConfigRead request build/send, reply reassembly + ACK, DR-with-resend-OK retry, OIR-terminal classification, per-chunk timing capture. Live inside `ActiveExchange::CdiDownload`. **2026-07-14 extension (ADR-0018)**: reply-identity check on `ReadReply::Success` (mismatched `address` vs `address_cursor` → ACK+discard+continue, no cursor advance, no fatal error); assembler-error recovery via `assembler.clear_source(self.alias)` when a stray `DatagramMiddle` arrives with no matching `First` in the current generation → keep the exchange in-flight, no `TerminateDueToError`.
  - `PeerSession::on_memory_read_frame` / `on_memory_write_frame` (+ `send_read_request` / `send_write_request`) ([lcc-rs/src/peer_session.rs](../lcc-rs/src/peer_session.rs)) — per-session config Contributors added in S4 (ADR-0018 §2026-07-18 extension). Single-datagram memory primitives living inside `ActiveExchange::MemoryRead` / `MemoryWrite`: read = one memory-config read round-trip (arbitrary `AddressSpace::from_u8`, reply-identity guard, sole-ACK-owner, returns `(Vec<u8>, MemoryReadTiming)`); write = RequestWithNoReply (`DatagramReceivedOk` = success, resend-OK DR retry to `WRITE_MEMORY_MAX_RETRIES`). Concurrent ops serialise via the unified `mem_pending` FIFO. D1=A: CDI-element batch planning stays app-side; only the single datagram exchange is in the actor.
- **Consumers** (callers that fetch a `PeerSessionHandle` and dispatch protocol commands):
  - `LiveNodeProxy::handle_query_snip` / `handle_query_pip` ([bowties-core/src/node_proxy.rs](../bowties-core/src/node_proxy.rs)) — fetch-per-call (D2). When `sessions: Some(Arc<PeerSessionRegistry>)`, the proxy calls `sessions.get(node_id).await` and delegates to `session.query_snip()` / `query_pip()`. When `sessions: None` (test paths without a registry, or pre-S2 wiring) it falls back to the free-function shim `lcc_rs::query_snip` / `pip::query_pip`. The proxy never caches a `PeerSessionHandle` — the registry is the sole source of truth.
  - Tauri `query_snip_single` / `query_snip_batch` / `query_pip_single` / `query_pip_batch` ([app/src-tauri/src/commands/discovery.rs](../app/src-tauri/src/commands/discovery.rs)) — indirect consumers via `NodeProxyHandle::query_snip` / `query_pip`, which dispatch through `LiveNodeProxy` above.
  - `PeerSession` health-forwarder task ([lcc-rs/src/peer_session.rs](../lcc-rs/src/peer_session.rs)) — per-session Consumer of the Transport Health seam (see above); dispatches `PeerCommand::TransportWedged` into its own command mpsc.
  - `commands::cdi::download_cdi` ([app/src-tauri/src/commands/cdi.rs](../app/src-tauri/src/commands/cdi.rs)) — Consumer added in S3 (ADR-0018). Resolves `PeerSessionHandle` via `state.sessions.read().await.get(node_id)` (fetch-per-call) and dispatches `PeerCommand::DownloadCdi` via the typed `handle.download_cdi(config)` method. Returns `CdiCompletion { bytes, stats }` synchronously — no `cdi-progress` event surface. `commands::cdi::cancel_cdi_download` iterates every registered session via `registry.snapshot_handles()` and dispatches `PeerCommand::Cancel` to each (S3 T6 sub-decision (ii) — global-cancel semantic preserved without an `AtomicBool` at the app layer).
  - `commands::cdi` config read/write + `commands::sync_panel` targeted read/write ([app/src-tauri/src/commands/cdi.rs](../app/src-tauri/src/commands/cdi.rs), [app/src-tauri/src/commands/sync_panel.rs](../app/src-tauri/src/commands/sync_panel.rs)) — Consumers added in S4 (D2=ii full closure). `read_config_value` / `read_all_config_values` / `read_memory_with_retry` / `write_config_value` (+ 2 more write sites) and the sync_panel targeted read (~485) + write (~958) all resolve a `PeerSessionHandle` via `AppState::peer_session(node_id)` (the single DRY fetch-per-call resolver) and dispatch `handle.read_memory(...)` / `handle.write_memory(...)`. Grep invariant: zero `batch_reader` and zero bare-`conn.read_memory` / `conn.write_memory` hits remain in `app/src-tauri`. The dead `BatchReader` machinery was deleted from `lcc-rs::discovery`.
  - `EventRouter` ([lcc-rs/src/event_router.rs](../lcc-rs/src/event_router.rs) — new in S6) — **planned in S6**. Bus-scoped peer of `PeerSessionRegistry`, subscribed to the same transport inbound broadcast; classifies event-report MTIs and fans out independently of sessions.
- **Per-slice plumbing rule**: `PeerSession::spawn` has exactly one production caller — the spawn-watcher task in `peer_session_registry.rs`. Grep for `PeerSession::spawn` across the workspace: any hit outside `peer_session_registry.rs` or `#[cfg(test)]` / `tests/**` violates the invariant. New protocol handlers (SNIP, PIP, CDI, config r/w, event routing) MUST add themselves as Consumers of an existing `PeerSessionHandle` fetched from the registry, never construct a session directly. The typed convenience methods on `PeerSessionHandle` (`query_snip`, `query_pip`, `download_cdi`, `read_memory`, `write_memory`, `cancel`) drop the registry read guard before the send/await — a raw `.command()` dispatch is also available for registry-internal control frames (`PeerReinitialised`, `AliasChanged`, `TransportWedged`).
- **Last-modified**: 2026-07-18 (S4 — config read/write migrated onto the actor as `ActiveExchange::MemoryRead` / `MemoryWrite` single-datagram primitives (`PeerSessionHandle::read_memory` / `write_memory`); `commands/cdi.rs` + `commands/sync_panel.rs` rewired via `AppState::peer_session`; dead `BatchReader` deleted from `lcc-rs::discovery`; closes the dual-path CDI-vs-config interleave regression [D1=A + D2=ii, ADR-0018 §2026-07-18]. Also S7 — inbound delivery moved to registry-owned per-peer `mpsc` demux; `InboundEvent::Lagged` drives bounded in-place CDI recovery; `abort_active` gate recast to `PeerError::is_our_fault_live_wire()`; D3 superseded for CDI)
- **Last-audited**: 2026-07-07

### Notes

**Sole-spawner qualification** (ADR-0016 invariant 3): only three MTIs carry a full NodeID and trigger session creation — `VerifiedNode` (0x19170), `InitializationComplete` (0x19100), and `AliasMapDefinition` (0x10701). Frames that carry only an alias (PCER, `EventReportWithPayload`, addressed exchange frames) never spawn a session. Delaying spawn to AMD-only would drop initialisation-time peers that announce via VNI before their first AMD; three-frame qualification with `AliasChanged` idempotence covers all observed lifecycles.

**Alias renegotiation is a peer-lifetime event, not a peer-identity event** (ADR-0016 rejected alternative): the registry is keyed by `NodeID`, not `(NodeID, alias)`. A second observation with a new alias updates `self.alias` in place inside the existing session via `PeerCommand::AliasChanged` — the actor task, its mpsc command channel, its inbound broadcast subscription, and its cached SNIP/PIP all survive. If the alias changes mid-exchange, the exchange is aborted with `PeerError::AliasChanged { old, new }` because the old alias is no longer routable; caches survive (they are value snapshots, not exchange state).

**Inbound lag policy** (ADR-0016 D3, superseded for CDI by the §2026-07-18 extension / S7): sessions no longer subscribe to the shared bus — the registry demux delivers a per-peer `InboundEvent::Lagged(pending)` only on its own bounded channel overflow (fan-out amplification is structurally gone). On `InboundEvent::Lagged` with an active `CdiDownload`, the session does **bounded in-place recovery** (reset assembler + re-issue same-cursor read, cumulative `lag_recovery_count` = `config.max_retries`) and aborts through the fault-nature cleanup gate — emitting exactly one `TerminateDueToError` — only on exhaustion. SNIP/PIP retain the original D3 abort-and-continue (`PeerError::Protocol`, caches preserved). The cleanup gate is `PeerError::is_our_fault_live_wire()` (not the old `matches!(err, Cancelled | Rejected)` allowlist), so any our-fault-live-wire abort emits cleanup; `Timeout` still emits via `handle_deadline` (exactly-once).

**Mid-exchange `Wedged`** (ADR-0016 D1): a per-session health-forwarder task translates the Transport Health seam's `Healthy → Wedged` transition into `PeerCommand::TransportWedged { reason }` on the session's own command mpsc. The `run()` loop aborts the active exchange with `PeerError::TransportUnhealthy` and **skips** the outbound `TerminateDueToError` cleanup emission — peer cleanup requires a live wire, and enqueuing on a wedged transport would hit the fail-fast in `TransportHandle::send()` anyway. Command-entry state is remembered via `last_known_wedge_reason` for S3+ fast-path use.

**F4 depth audit — `LiveNodeProxy` retained** (S2 slice exit, 2026-07-07): after moving SNIP/PIP coalescing into `PeerSession`, `LiveNodeProxy` still owns bounded app-layer responsibility: `last_seen` / `last_verified` / `connection_status` observation, the `NodeProxyHandle::Live | Synthesized` polymorphism required by ADR-0009, the `DiscoveredNode` snapshot facade that fuses SNIP cache + app-layer state, externally-driven SNIP/PIP cache updates from `EventRouter`, and delegation to the session for protocol calls. These are app-layer concerns not protocol concerns; folding into `NodeRegistry` would collapse two distinct roles. No follow-up idea issue.

**Test coverage** (`lcc-rs/tests/peer_session_snip_pip.rs`):
- `cold_query_snip_returns_data` / `cold_query_pip_returns_flags` — round-trip through the session.
- `two_concurrent_query_snip_coalesce_to_single_wire_request` / `two_concurrent_query_pip_coalesce_to_single_wire_request` — coalescing verified by outbound-frame counting.
- `snip_cache_hit_makes_no_wire_request` — cache hit emits no additional outbound frame.
- `peer_reinitialised_clears_snip_and_pip_caches` — `PeerCommand::PeerReinitialised` clears both caches; subsequent queries re-emit wire requests.
- `alias_changed_updates_outbound_destination` — `PeerCommand::AliasChanged` updates the session's outbound destination in place with no duplicate spawn.
- `registry_spawns_on_vni_and_updates_alias_on_amd` / `registry_spawns_on_init_complete_and_amd` — spawn qualification for all three NodeID-carrying MTIs.
- `registry_delivered_handle_supports_query_snip` — a handle fetched from the registry (spawned via AMD) supports typed queries end-to-end.
- `broadcast_lagged_aborts_active_exchange_and_preserves_caches` — abort-then-cache-survives contract.
- `inbound_lag_aborts_active_pip_and_preserves_snip_cache` (S7) — a real `InboundEvent::Lagged` during an active PIP aborts it (`PeerError::Protocol`) while the seeded SNIP cache survives.
- `cancel_returns_cancelled_error` — `PeerCommand::Cancel` returns promptly with `PeerError::Cancelled`.

S7 (2026-07-18) added, in `lcc-rs/tests/peer_session_cdi.rs`: `cdi_mid_download_lag_recovers_in_place_and_completes` (AC1+AC3 — mid-CDI lag resets the assembler, re-issues the same cursor, completes with correct bytes, zero `TerminateDueToError`) and `cdi_sustained_lag_storm_exhausts_recovery_then_cleans_up` (AC2 — the 4th lag with `max_retries=3` exhausts recovery → abort + exactly one `TerminateDueToError` to the correct dest alias). In `lcc-rs/tests/peer_session_registry.rs`: `construction_window_frame_before_route_is_dropped` (the DROP contract, proven via a registered sync peer establishing happens-before). In `lcc-rs/src/peer_session.rs` `classifier_tests`: `is_our_fault_live_wire` covers live-wire faults and excludes wire-dead / peer-initiated.