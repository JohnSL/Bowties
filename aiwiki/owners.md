...
| File | Purpose | Test |
|------|---------|------|
| `RailroadPanel.svelte` | **Spec 018 / S3 → S4:** outer Railroad-tab composer. Renders `<FacilitiesSection>` (Spec 018 / S1) above the new `<ChannelsPanel>`. Threads `nodeName`, `resolvedEventIds`, optional `daughterboardName(nodeKey, connector)`, plus (S4) the `usedBy(channelId)` resolver and the `onSelectChannel` / `onRebindChannel` / `onRemoveFromSlot` slot-binding intent callbacks through to `ChannelsPanel` and `FacilitiesSection`. `+page.svelte` mounts only `<RailroadPanel>` on the Railroad tab (no longer a sibling `<FacilitiesSection>` mount). | `RailroadPanel.integration.test.ts`, `page.route.test.ts` |
...
| File | Purpose | Test |
|------|---------|------|
| `FacilitiesSection.svelte` | **Spec 018 / S1 → S4:** Railroad-tab section housing the Add-facility button + facility list. Owns its own `showAddDialog` state for `AddFacilityDialog`. **S4 additions:** accepts `resolvedEventIds`, `onSelectChannel`, `onRebindChannel`, `onRemoveFromSlot` props from `RailroadPanel` and forwards them to each `<FacilityCard>`. Mutates `facilitiesStore` only for facility-level CRUD (add/rename/delete); slot-binding intent bubbles up to the route (no direct `facilitiesStore.attachChannel` calls). | (covered by `RailroadPanel.integration.test.ts`, `selectChannel.integration.test.ts`) |
| `FacilityCard.svelte` | **Spec 018 / S1 → S4 (mockup-aligned):** one facility = one card with a header (title group: name · template label · filled status pill with pulse dot · Rename/Delete link actions) + a **horizontal slot grid** (`1fr 28px 1fr`) with `→` arrows between adjacent slots; single-slot templates collapse to one column via a `:has(...)` rule. Click-to-rename name input. Status derivation: `Object.values(slotBindings).every(v => v.length > 0) ? 'Wired' : 'Incomplete'` (plural shape per D8). Computes the filled-state display per slot from `channelsStore.channels` + `eventStateStore.events` + the route-supplied `resolvedEventIds` (picks element 0 of the Vec since UI is max-1 today) and threads `channel.ownership` through `currentChannelDisplay.ownership` so the slot can render the `HW`/`USER` badge. Threads slot intent callbacks to each `<FacilitySlot>` and prepends `facilityId` before bubbling. | (covered by `selectChannel.integration.test.ts`) |
| `FacilitySlot.svelte` | **Spec 018 / S1 → S4 (mockup-aligned):** one slot inside a facility card, rendered as a vertical card with an uppercase `{Label} (input)`/`{Label} (output)` header (kind suffix derived from `SlotDefinition.kind`: producer→input, consumer→output). Empty state (mockup §3): italic "empty" + right-aligned `Select channel…` button (dashed border, lighter background). Filled state (mockup §6): state dot · channel name · `HW`/`USER` ownership badge · `{groupLabel} · {locationLabel} · {state}` meta line + `Rebind…` / `Remove from slot` link actions (no Rename per D6 — channel rename lives in the Channels panel). Accepts `currentChannelId?`, `currentChannelDisplay?` (with `ownership: 'hardware-owned' \| 'user-owned'`), plus `onSelectChannel(slot)` / `onRebindChannel(slot, currentId)` / `onRemoveFromSlot(slot, currentId)` callbacks. Pure render + emit; no store reads/writes. | (covered by `selectChannel.integration.test.ts`) |
| `SelectChannelPicker.svelte` | **Spec 018 / S4 (new):** modal picker for Select-channel and Rebind workflows (mockup §5). Built on the `Dialog/` shell (alertdialog, focus trap, Esc / overlay / × → onCancel). Props: `{ slotLabel, requiredRole, currentChannelId?, candidateChannels, channelState, onConfirm, onCancel }`. Renders a search input filtering by name or location, a radio list with state-dot + name + role/location/state meta, and Cancel + Confirm actions. Confirm is disabled until a selection exists; in Rebind mode (`currentChannelId` set) it stays disabled until a *different* channel is selected. The route owns picker open/close + mode state and dispatches `onConfirm` through `facilityOrchestrator.selectChannelForSlot`. | `SelectChannelPicker.test.ts` |
...
| `facilityOrchestrator.ts` | **Spec 018 / S4 (new) — Owner of the Slot Binding seam.** Pure workflow module (no DOM, no IPC): `selectChannelForSlot({ facilityId, slotLabel, channelId, mode, previousChannelId? })` validates `channel.role === slot.requiredRole` via `behaviorTemplatesStore` and on `mode: 'rebind'` runs detach-then-attach atomically (no separate "rebind" delta — D3/D4). `removeFromSlot({ facilityId, slotLabel, channelId })` calls `facilitiesStore.detachChannel`. Throws `RoleMismatchError` on role mismatch (before any store write) and `UnknownReferenceError` when the facility/slot/channel/template lookup misses. Component layers MUST go through this orchestrator — they do not call `facilitiesStore.attachChannel` / `detachChannel` directly. | `facilityOrchestrator.test.ts` |
...
| `channels.svelte.ts` | `channelsStore` singleton (Spec 015). Holds `InformationChannel[]` for the active layout. Exposes `channels`, `isEmpty`, `grouped` (by **role** per Spec 018 / S2), `groupedByHardware` (**Spec 018 / S3** — by node + subsystem via the module-level `hardwareGroupKey(channel)` helper; `connectorInput` channels group per `${nodeKey}|connector:${connector}`, `lampRow` channels group per `${nodeKey}|direct-lamp-control`), `loading`, `isDirty`, `editCount`, `pendingCreations`, `pendingRenames`, `pendingDeletions`; mutators `loadChannels()` (IPC), `addPendingChannels(ch[])`, `deleteChannels(ids[])`, `renameChannel(id, newName)` (rejects empty; returns bool), `setChannels(ch[])`, `discard()`, `hydrateBaseline(ch[])`, `reset()`. Draft layer (ADR-0012): tracks `_pendingCreations`, `_pendingRenames`, and `_pendingDeletions`; `channels` getter filters deletions and applies renames over baseline + creations. Hydrated on layout open (eager); reset on layout close via `layoutLifecycleOrchestrator`. **Spec 018 / S1.2 (ADR-0011 extension 2026-06-28):** `editCount` feeds `effectiveNodeStore.dirtyBreakdown.channels` so the Save / Discard toolbar and the close-prompt dialog react to channel-only edits. **Spec 018 / S2 (ADR-0013):** `InformationChannel` now carries `{role, style, ownership, binding}` instead of `{channelType, hardwareRef}`; `grouped` keys by `role`. | `channels.svelte.test.ts`, `channels.s2.integration.test.ts` |
...
| `channelStyles.ts` | **Spec 018 / S2 (ADR-0013):** frontend style registry. Maps style id → producer event-leaf mapping (`{state: {producerLeafIndex}}`). Exports `getStyleEventMapping(styleId)` returning `undefined` for unknown styles. Single source of truth for the lookup the `eventStateOrchestrator` and `+page.svelte` perform per channel; replaces the retired hardcoded `BOD_EVENT_MAPPING` constant. S2 ships one entry (`bod-block-detector-input`); S5 adds `single-led-direct-lamp`. | `channelStyles.test.ts` |
| `channelState.ts` | **Spec 016 / S1:** Pure `deriveChannelState(events, occupiedId, clearId)` → `'no-config' \| 'unknown' \| 'occupied' \| 'clear'`. Compares timestamps from the event state store to determine live channel occupancy. **Spec 017 / S2:** Added `'no-config'` for channels whose event IDs cannot be resolved (placeholder, off-bus node, partial-capture saved tree) — distinct from `'unknown'` (resolved but no event seen). | `channelState.test.ts` |
...
| `channels.ts` | Information channel inventory IPC (Spec 015): `listChannels`, `createChannels`, `renameChannel`, `deleteChannels`. Backend persists to `channels.yaml`. Called by save workflow (create/rename/delete) and by `channelsStore.loadChannels()` (list). **Spec 018 / S2 (ADR-0013):** `InformationChannel` now carries `{role, style, ownership, binding}` (discriminated union: `connectorInput` today, `lampRow` in S5) instead of `{channelType, hardwareRef}`; also exports `ChannelRole`, `ChannelOwnership`, `ChannelBinding` type aliases. |
...
| `node_tree.rs` | Unified config tree: CDI + addresses + values + roles. Also owns `NodeRoles` (per-event producer/consumer sets). **Event ID canonical form:** `parse_event_id_hex()`, `normalize_event_id_hex()`, `bytes_to_display_hex()` are thin delegations to `lcc_rs::EventID` (`from_hex_string`, `to_canonical`, `to_hex_string`). The byte ↔ hex string rule lives in `lcc_rs::types`; nothing in `bowties-core` reimplements it. See ADR-0010. | inline `#[cfg(test)]` (24+) |
| `node_proxy.rs` | Polymorphic node handle (`NodeProxyHandle` enum: `Live` + `Synthesized`). `LiveNodeProxy` actor; `SynthesizedNodeProxy` passive state holder. **Slice 3a (2026-06-28, ADR-0015 + ADR-0009 amendment):** `LiveNodeProxy` no longer holds CDI bytes — the `cdi_data` / `cdi_parsed` fields and their `GetCdiData` / `SetCdiData` / `GetCdiParsed` / `SetCdiParsed` `ProxyMessage` variants are gone. `LiveNodeProxy::snapshot()` always emits `cdi: None`. `NodeProxyHandle::get_cdi_data` / `get_cdi_parsed` return `Ok(None)` for `Live` and the field value for `Synthesized` (placeholder CDI still lives in the proxy struct, since there is no `LayoutState` entry for unsaved placeholders). `set_cdi_data` / `set_cdi_parsed` removed from the enum entirely — every former caller now records into `LayoutState.captured` via `record_captured`. **Retained working buffers on `LiveNodeProxy`** (deliberately, not duplication): `snip`, `pip_flags`, `config_values`, `config_tree`. These are per-actor live working state for in-progress bus operations (SNIP/PIP queries, partial-read accumulators, `set_modified_value` pending writes); moving them to `LayoutState` would introduce a new concurrency surface for in-flight reads vs. save snapshots. | inline `#[cfg(test)]` |
| `node_registry.rs` | Thread-safe `NodeKey → NodeProxyHandle` map. Owns `saved_trees`, a **load-once seeding cache** of profile-annotated trees populated by `open_layout_directory` and consumed exactly once when `get_or_create` spawns a fresh `LiveNodeProxy` for a rediscovered node — not a parallel source of truth for the save flow (which now reads from `LayoutState`). **Contract (Spec 017 / S3):** trees stored in `saved_trees` must be profile-annotated (`profile_applied = true`, `event_role` set on producer leaves); the same trees are also stored in `LayoutState.saved[].tree`, sourced from the identical loop in `app/src-tauri/src/commands/layout_capture.rs::open_layout_directory`. | inline `#[cfg(test)]` |
...
| `layout/types.rs` | YAML data structures, `ConnectionConfig`, `LayoutEditDelta` (incl. `AttachChannelToSlot` / `DetachChannelFromSlot` per Spec 018 / S4), `apply_layout_deltas` (no-ops on facility deltas; the work is in `layout/facilities.rs::apply_facility_deltas`). | inline `#[cfg(test)]` |
...
| `layout/state.rs` | **`LayoutState` deep module** (ADR-0015). Single in-memory owner of the open layout's three-layer projection: `saved` (mirrors disk), `captured` (fresh-from-bus not yet persisted), `drafts` (frontend edits awaiting save). Public API: `from_loaded(root, LayoutDirectoryReadData, cdi_xml_by_key, trees_by_key)`, queries (`persisted_node_keys`, `saved_node`, `captured_node`, `cdi_xml`, `config_tree`, `snapshot_for_save`), mutations (`record_captured`, `merge_drafts`, plus mut accessors for slice-2 expansion). **Captured-vs-saved precedence (slice-1 contract):** `cdi_xml(key)` and `config_tree(key)` always prefer captured over saved. **R1/R2 behavior pins:** `r1_every_persisted_node_resolves_cdi_xml_after_open` (open-with-CDI-map invariant) and `r2_captured_cdi_resolves_for_unsaved_node` (record_captured wire-up). **Slice 3a (2026-06-28):** the slimmed-`NodeProxy` consolidation landed — `LayoutState` is now the sole in-memory home for an open layout's persistent CDI XML, config values, and config trees. `AppState.offline_bowtie_data` was deleted; the offline catalog rebuild in `app/src-tauri/src/commands/bowties.rs` derives its projections directly from `LayoutState.saved`. `node_registry.saved_trees` is retained as a one-shot proxy-seeding cache (not a save-flow source of truth); `LiveNodeProxy::cdi_data` / `cdi_parsed` and their `ProxyMessage` variants were removed. Original design rationale in [specs/archive/backend-layout-state.md](../specs/archive/backend-layout-state.md) (historical reference only). | inline `#[cfg(test)]` (6 tests) |
