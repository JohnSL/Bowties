...
| File | Purpose | Test |
|------|---------|------|
| `RailroadPanel.svelte` | Tab panel for channel inventory. Reads `channelsStore` + `eventStateStore`; renders grouped channels with derived occupancy states or empty-state guidance. Receives `nodeName` and optional `resolvedEventIds` props. Computes channel states reactively via `deriveChannelState()`. | `channels.svelte.test.ts` (store), `page.route.test.ts` (tab integration) |
...
| `channels.svelte.ts` | `channelsStore` singleton (Spec 015). Holds `InformationChannel[]` for the active layout. Exposes `channels`, `isEmpty`, `grouped` (by type), `loading`, `isDirty`, `editCount`, `pendingCreations`, `pendingRenames`, `pendingDeletions`; mutators `loadChannels()` (IPC), `addPendingChannels(ch[])`, `deleteChannels(ids[])`, `renameChannel(id, newName)` (rejects empty; returns bool), `setChannels(ch[])`, `discard()`, `hydrateBaseline(ch[])`, `reset()`. Draft layer (ADR-0012): tracks `_pendingCreations`, `_pendingRenames`, and `_pendingDeletions`; `channels` getter filters deletions and applies renames over baseline + creations. Hydrated on layout open (eager); reset on layout close via `layoutLifecycleOrchestrator`. **Spec 018 / S1.2 (ADR-0011 extension 2026-06-28):** `editCount` feeds `effectiveNodeStore.dirtyBreakdown.channels` so the Save / Discard toolbar and the close-prompt dialog react to channel-only edits. | `channels.svelte.test.ts` |
| `facilities.svelte.ts` | `facilitiesStore` singleton (Spec 018 / S1). Full ADR-0012 draft-layer contract: `facilities`, `isEmpty`, `isDirty`, `editCount`, `pendingCreations`, `pendingRenames`, `pendingDeletions`; mutators `addFacility(template, name)` (materialises empty slot bindings from template), `renameFacility(id, newName)` (no-op suppression against effective view; reverts pending rename when user matches baseline), `deleteFacility(id)` (removes pending creation OR records baseline deletion), `loadFacilities()`, `discard()`, `hydrateBaseline(facilities)`, `reset()`; **`collectDeltas(): LayoutEditDelta[]`** producing `addFacility / renameFacility / deleteFacility` variants (D1 = B per S1 HITL review). Hydrated on layout open in `+page.svelte`; reset on layout close via `layoutLifecycleOrchestrator`. **Spec 018 / S1.2:** `editCount` feeds `effectiveNodeStore.dirtyBreakdown.facilities`; `SaveControls.handleConfirmDiscard` calls `facilitiesStore.discard()` alongside the other store discards. | `facilities.svelte.test.ts` |
| `behaviorTemplates.svelte.ts` | `behaviorTemplatesStore` singleton (Spec 018 / S1). **Application-scoped** (NOT layout-scoped per D4): loaded once at app start via `loadBehaviorTemplates()` IPC; never enrolled in `layoutLifecycleOrchestrator.resetForNewLayout()`. Read-only mirror of the backend `bowties_core::behavior_templates` registry. Exposes `templates`, `loaded`, `findByTemplateId(id)`; `reset()` exists for tests only. | (covered by `facilities.svelte.test.ts` integration) |
...
| `node_registry.rs` | Thread-safe `NodeKey → NodeProxyHandle` map. Owns `saved_trees` cache. **Contract (Spec 017 / S3):** trees stored in `saved_trees` must be profile-annotated (`profile_applied = true`, `event_role` set on producer leaves) — the seam responsible is `app/src-tauri/src/commands/layout_capture.rs::open_layout_directory`. `register_node` seeds the live proxy directly from `saved_trees`, so a non-annotated tree there propagates silently to every reader. | inline `#[cfg(test)]` |
...
| `behavior_templates.rs` | **Behavior template registry** (Spec 018 / S1) — hardcoded `BehaviorTemplate` / `SlotDefinition` / `SlotKind` / `StateMapping` types; the `BLOCK_INDICATOR` constant + `registered_templates()` + `find_template()` accessors. Serialize-only (frontend has its own mirror type); no `Deserialize` because the registry is code-level, not loaded from YAML. Exposed to the frontend by the `list_behavior_templates` IPC. | inline `#[cfg(test)]` (5 tests) |
