...
- **Pattern**: Working buffers on a per-actor mailbox (e.g. `LiveNodeProxy::config_tree` for in-flight read accumulation, `config_values` for `set_modified_value` pending writes) are legitimately scoped to the actor and do NOT need to move into `LayoutState`. The principle test that motivated the slice was "is this a duplicate cache of *persistent* data that the save flow could read out-of-sync?" — the removed fields were yes; the retained fields are working memory for in-progress bus operations and have no save-path consumer.
