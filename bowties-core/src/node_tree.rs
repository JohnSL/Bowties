...
// ─────────────────────────────────────────────────────────────────────────────
// Protocol event role data
// ─────────────────────────────────────────────────────────────────────────────

/// Per-event producer/consumer classification collected from protocol replies.
///
...
pub struct NodeRoles {
    /// Node keys that replied ProducerIdentified for this event.
    pub producers: std::collections::HashSet<NodeKey>,
    /// Node keys that replied ConsumerIdentified for this event.
    pub consumers: std::collections::HashSet<NodeKey>,
}
...
pub struct NodeConfigTree {
    ...
    /// Configuration-mode selections that referenced an undeclared variant
    /// (Spec 014 / S6). Surfaced inline in the editor's mode selector so the
    /// user knows the saved selection is invalid without parsing free-form
    /// warning strings.
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub unknown_variants: Vec<crate::profile::UnknownVariantWarning>,
    /// Whether profile metadata (event roles, connector profile, relevance
    /// rules) has been applied to this tree. Used by `get_node_tree` to
    /// decide whether the fast-path cached tree needs annotation before
...
pub fn classify_leaf_roles_from_protocol(
    ...
    let node_key = crate::node_key::NodeKey::parse(&tree.node_id)
        .unwrap_or_else(|_| crate::node_key::NodeKey::from_node_id(
            lcc_rs::NodeID::from_hex_string(&tree.node_id)
    ...
    for leaf_info in collect_event_id_leaves(tree) {
        if let Some(bytes) = leaf_info.value {
            if let Some(roles) = event_roles.get(&bytes) {
                let is_producer = roles.producers.contains(&node_key);
                let is_consumer = roles.consumers.contains(&node_key);
...
mod tests {
    ...
    fn classify_leaf_roles_producer_only() {
        ...
        event_roles_map.insert(event_bytes, NodeRoles {
            producers: [crate::node_key::NodeKey::parse("05.02.01.02.03.00").unwrap()].iter().cloned().collect(),
    ...
    fn classify_leaf_roles_consumer_only() {
        ...
        event_roles_map.insert(event_bytes, NodeRoles {
            ...
            consumers: [crate::node_key::NodeKey::parse("05.02.01.02.03.00").unwrap()].iter().cloned().collect(),
        });
        let result = classify_leaf_roles_from_protocol(&tree, &event_roles_map);
    ...
    fn classify_leaf_roles_both_is_ambiguous() {
        ...
        let node_key = crate::node_key::NodeKey::parse("05.02.01.02.03.00").unwrap();
        let mut event_roles_map = HashMap::new();
        event_roles_map.insert(event_bytes, NodeRoles {
            producers: [node_key].iter().cloned().collect(),
            consumers: [node_key].iter().cloned().collect(),
        });
        let result = classify_leaf_roles_from_protocol(&tree, &event_roles_map);
    ...
    fn classify_leaf_roles_uninvolved_not_included() {
        ...
        let mut event_roles_map = HashMap::new();
        event_roles_map.insert(event_bytes, NodeRoles {
            producers: [crate::node_key::NodeKey::parse("99.99.99.99.99.01").unwrap()].iter().cloned().collect(),
            consumers: std::collections::HashSet::new(),
        });
        let result = classify_leaf_roles_from_protocol(&tree, &event_roles_map);
