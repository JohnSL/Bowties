...
A new root-level `handbook/` area will hold the current behavioral and architectural truth for the application.

Rules:

- `handbook/` documents current expected behavior and active architecture boundaries
- `specs/` documents how a feature was planned and built
- when `specs/` and `handbook/` disagree, `handbook/` wins unless the code has been intentionally changed and the handbook is stale
- when behavior changes intentionally, tests and `handbook/` should be updated together
...
- `handbook/README.md`
- `handbook/user-stories/offline-sync-workflows.md`
- `handbook/architecture/frontend-boundaries.md`
- `handbook/quality/engineering-principles.md`
- `handbook/quality/testing-strategy.md`
...
- treat `handbook/` as the durable source of truth for current behavior and architecture
...
- when behavior changes, update tests and `handbook/` together
...
- `maintain-current-docs`
  - update `handbook/` after approved behavior or architecture changes
...
The following should be documented in `handbook/architecture/frontend-boundaries.md` and reinforced in instructions.
...
The handbook should continue to describe durable documentation and governance rules. The spec-local test plan should continue to describe:
...
1. Create `handbook/README.md`.
2. Create `handbook/quality/engineering-principles.md`.
3. Create `handbook/architecture/frontend-boundaries.md`.
...
6. Add file-scoped instructions for `handbook/**` and frontend boundary areas.
...
14. Update `handbook/` whenever behavior or ownership boundaries change.

## Approval Checklist

Before implementation, confirm these decisions:

- `handbook/` is accepted as the durable source of truth for current behavior and architecture
...
- confirm whether `handbook/` structure and naming are still approved
