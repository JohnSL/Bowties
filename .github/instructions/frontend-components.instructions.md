...
- When touching a component that already mixes rendering with workflow or state ownership, improve that separation as part of the change instead of adding to the overlap.
...
- Apply TDD by covering rendering and emitted intent behavior with focused component tests, and add missing regression tests when a touched component previously lacked them.