...
- Q: How should reverting an offline change interact with disk persistence and the save/discard lifecycle? → A: A revert is treated as a pending in-memory modification, identical in lifecycle to any other edit — it is not auto-written to disk. All pending changes (including reverts) are committed only on explicit user save and can be discarded before saving. There is no distinction between "persisted" and "draft" offline changes for save semantics.
- Q: What happens to the node tree and configuration views when the user disconnects while a layout is open? → A: The tree and config views must NOT clear, regardless of whether the layout was in online or offline mode. If the layout has captured snapshots, the tree is re-hydrated from those. If no layout is open at disconnect time, the app transitions to the connection dialog.
...
6. **Given** the user wants to revert a specific offline change, **When** they revert a field to its captured value, **Then** the pending change is removed from in-memory state and the field shows the captured baseline value; the revert is not committed to disk until the user explicitly saves the layout.
7. **Given** the user has reverted a field to its captured value but has not yet saved, **When** they click Discard, **Then** the revert itself is undone and the field returns to the previously pending offline value, confirming that reverts are pending modifications until saved.
...
- **What if the user reverts an offline change?** A revert — restoring a field's planned value to its captured baseline — is treated as a pending in-memory modification identical in lifecycle to any other edit. The revert removes the pending change from in-memory state and clears the leaf's `modifiedValue` in one operation, so the field shows the captured baseline immediately. However, the revert is committed to disk only when the user explicitly saves the layout. Before saving, the user can discard the revert (along with any other unsaved modifications) to restore the previously pending offline value. There is no "persisted vs. draft" distinction for purposes of the save lifecycle; all unsaved changes are discardable until save.
- **What if nodes without CDI (fingerprint `"not_supported"` or `"missing"`) are included in a layout save?** Their YAML snapshot files will be unreadable on reload and produce "(Not captured)" banners for every field. Such nodes must be excluded from the snapshot write entirely during `save_layout_directory`.
- **What if the user dismisses the Sync Panel and then reconnects or changes conditions that would normally trigger it?** Auto-triggers from the discovery settle timer must NOT re-open the panel after the user has dismissed it. Only an explicit user action — via the "Open Sync Panel" button in the offline banner or the "Sync to Bus" menu item — may re-open it. The dismissed state must persist until the user explicitly requests re-opening.
- **What if the user disconnects while a layout is open (online or offline)?** The node tree and configuration views must not clear or collapse. If the layout has captured snapshots, the app re-hydrates the tree from those snapshots so the user can continue browsing. If the layout was opened online without a prior save (no captured snapshots), values read in the current session remain visible but are no longer refreshable until reconnection. If no layout is open when the connection is dropped or closed, the app transitions to the connection dialog.
- **What if a config field has a pending persisted offline change and the user goes online?** The field's `modifiedValue` must be set to the planned offline value (not null) as soon as the live tree loads, so the field displays "pending" state rather than the raw bus value. The annotation must show both the bus baseline and the planned value side-by-side ("Bus: X | Pending: Y").
- **What happens to the on-disk node snapshot for a field that was successfully applied during sync?** After `apply_sync_changes` completes, the applied field's new value is the authoritative current state of the bus. The node snapshot in the companion directory must be updated to reflect the applied values so that if the user disconnects and re-opens the layout offline, the snapshot shows the post-apply state as the baseline — not the pre-apply capture value. Snapshot updates are written atomically per-node as part of the apply confirmation step. Fields that were skipped, deselected, or failed are left with their existing snapshot values.
...
- **FR-020**: Reverting an offline change (restoring a field to its captured baseline) MUST be treated as a pending in-memory modification with the same save lifecycle as any other edit. The revert MUST remove the pending change from in-memory state and clear the leaf's `modifiedValue` in the node tree in one operation so the field shows the baseline immediately. The revert is committed to disk only on explicit user save; before saving, the user MUST be able to discard the revert (along with all other unsaved modifications) to restore the previously pending offline value. There is no separate auto-write path for reverts.
- **FR-023**: Layout save MUST exclude nodes whose CDI fingerprint is `"not_supported"` or `"missing"` from the node snapshot output. Saving YAML for such nodes produces unreadable placeholder files that render as "(Not captured)" on reload; those nodes should produce no YAML at all.
- **FR-024**: When a live node tree is loaded while a layout has pending persisted offline changes for that node, the system MUST immediately apply the planned offline values as `modifiedValue` on the matching leaves so that fields display the pending planned value (not the raw bus value). The annotation for such a field MUST show both the bus baseline value and the planned offline value (for example "Bus: X | Pending: Y") so the user can distinguish the two.
- **FR-025**: The Sync Panel MUST offer an explicit "Sync to Bus" re-entry affordance — both as a menu item and as a button in the offline status banner — that is available whenever the user is connected to the bus, has an offline layout with pending changes active, and has previously dismissed the Sync Panel. Auto-triggers from the discovery settle timer MUST NOT re-open the panel after a user dismissal; only the explicit affordance may do so.
- **FR-026**: When the user disconnects (or loses connection) while any layout is open — whether previously online or offline — the app MUST NOT clear the node tree or configuration views. If the layout has captured snapshots, the app MUST re-hydrate the tree from those snapshots so the user can continue browsing configuration values. If no layout is open at the time the connection is dropped or closed, the app MUST transition to the connection dialog.
- **FR-032**: Bowtie offline changes (names, tags, connections) are applied to the layout files on Apply and do not require any bus write for metadata-only changes; event ID changes in bowties DO require bus writes and must go through the same sync path as configuration edits.
- **FR-033**: The system MUST NOT automatically push any offline changes to the bus without explicit user action in the Sync Panel.
- **FR-034**: System MUST allow users to add staged nodes that were not present during original capture and persist them as first-class node files in the layout directory.
- **FR-035**: Staged nodes MUST remain marked as pending/not-validated until they are observed on a bus and either synced or explicitly acknowledged by the user.
- **FR-036**: Layout directory structure and file naming MUST produce meaningful git diffs where changes to one node are isolated primarily to that node's file.
- **FR-037**: If Close Layout is requested while unsaved or pending offline changes exist, the system MUST require explicit user choice to Save, Discard, or Cancel before closing.
- **FR-038**: Configuration values for connected nodes MUST only be shown as editable after a successful read for that node (either via per-node read or read-all). Nodes without successful reads MUST remain identity-only.
