...
- The next defect fixed under this plan was the sync-session already-applied stale-state gap: session build now clears already-applied rows from backend cache and the frontend reloads offline changes when that auto-clear occurs.
- The next defect fixed under this plan was the post-apply snapshot refresh gap: sync apply now updates both the applied baseline value and the snapshot `captured_at` timestamp, with helper-level unit coverage in `app/src-tauri/src/layout/node_snapshot.rs`.
- The next defect fixed under this plan was CDI preflight error classification: only `CdiNotRetrieved` now feeds the download prompt, while non-downloadable CDI failures surface as differentiated error-banner messages, covered by `app/src/lib/orchestration/configReadOrchestrator.test.ts` and `app/src/routes/page.route.test.ts`.
...
No remaining review-only gaps are open in this regression pass.
...
4. Record any newly discovered review-only gaps here until they are either fixed or intentionally deferred.
