...
- [ ] T031 [US3] Implement `set_offline_change` and `revert_offline_change` commands in `app/src-tauri/src/commands/layout_capture.rs`
- [ ] T032 [P] [US3] Persist pending offline change rows in `offline-changes.yaml` via `app/src-tauri/src/layout/io.rs`
- [ ] T033 [P] [US3] Add offline-change visual markers distinct from online dirty state in `app/src/routes/+page.svelte`
- [ ] T034 [US3] Extend bowtie edit flow to emit offline change rows in `app/src-tauri/src/commands/bowties.rs`
- [ ] T035 [US3] Persist offline bowtie metadata edits in `app/src-tauri/src/layout/io.rs`
- [ ] T036 [US3] Implement offline-save path preserving baseline/planned separation in `app/src-tauri/src/commands/layout_capture.rs`
...
- [ ] T055 Implement legacy layout migration entrypoint for older single-file persistence in `app/src-tauri/src/layout/manifest.rs`
