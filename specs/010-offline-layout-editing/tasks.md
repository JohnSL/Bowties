...
**Refactoring Tracker**: `refactoring-roadmap.md` (thin-slice refactor tasks + future architecture backlog)
...
- [x] T047m [US3] **Fix T047b** — Remove the `persist_offline_changes` auto-write call from `revert_offline_change` in `app/src-tauri/src/commands/sync_panel.rs`. Revert must remove the row from in-memory state only and signal the frontend to mark the layout dirty. The on-disk `offline-changes.yaml` is updated only when the user explicitly saves the layout, not at revert time.
- [x] T047n [US3] **Fix T047c** — Restore `layoutStore.markDirty()` in the revert onclick handler in `app/src/lib/components/ElementCardDeck/TreeLeafRow.svelte`. A revert is a pending modification until saved; the layout must be dirty so Save/Discard buttons appear and the user can either commit or undo the revert.
- [x] T047o [US3] **New** — Introduce saved-vs-in-memory layering to the offline changes store (`app/src/lib/stores/offlineChanges.svelte.ts`): track the last-saved snapshot of offline change rows separately from current in-memory rows. Discard must restore in-memory rows to the last-saved snapshot, which re-instates any planned offline values that were removed by an unsaved revert (US3 AS-7). Save must promote in-memory rows to the saved snapshot and write to disk.
- [x] T047p [US4] **Fix T047f** — Extend the disconnect handler in `app/src/routes/+page.svelte` to preserve the node tree for online-mode layouts as well as offline-mode layouts: when any layout is open at disconnect, re-hydrate from captured snapshots (or leave session-read values visible if no snapshots exist). When no layout is open at disconnect, transition to the connection dialog.
- [x] T047q **Tests** — Update `TreeLeafRow.offline.test.ts` to reflect restored `markDirty` on revert and no backend IPC call at revert time. Update `offlineChanges.store.test.ts` to cover the saved-vs-in-memory layering: discard restores a reverted planned value, save promotes in-memory to saved snapshot; add a test for the no-layout-open disconnect → connection dialog path.
