...
- Tower-LCC's source CDI XML is missing from the repo. **Resolution**: backfill from the connected hardware's `<cdi>` segment dump into `profile-extractions/tower-lcc/tower-lcc_cdi.xml` and bundle into `app/src-tauri/profiles/RR-CirKits_Tower-LCC.cdi.xml` during this work (per proposal Pointers + R6).
