...
- The **role** is what the channel does in the layout — its state vocabulary plus the slot-binding contract. Examples: `block-occupancy` (states `unknown` / `occupied` / `clear`), `lamp-indicator` (states `unknown` / `lit` / `unlit`), `signal-aspect` (states parameterized by the signal system — a 3-aspect channel has `unknown` / `stop` / `approach` / `clear`; a 5-aspect channel adds `restricting` / `medium-clear`; a European distant-signal channel might have `expect-stop` / `expect-proceed` / `expect-slow`). The `signal-aspect` role is a single role whose state vocabulary is declared per channel or per template slot, not a family of fixed roles (`signal-aspect-3-color`, `signal-aspect-5-color`, etc.). This matches how JMRI models signal systems: aspects are semantic (what to communicate), not physical (which LEDs to light). Facility slots bind **by role** — any channel of the slot's required role can fill it, regardless of which style implements it, as long as the channel's aspect vocabulary covers the aspects the template produces. State vocabularies name real-world intent (`occupied` / `clear`, `lit` / `unlit`), never electrical or boolean abstractions (`true` / `false`, `on` / `off`).
- The **style** is the specific hardware shape that realises a role on a specific subsystem — pins claimed, event-leaf mapping, CDI-field constraints, and (for `signal-aspect` channels) the **aspect-to-event map** that translates each abstract aspect into concrete hardware actions. Examples: `bod-block-detector-input` (1 input pin on a BOD daughter board, realises `block-occupancy`); `single-led-direct-lamp` (1 Direct Lamp Control row, realises `lamp-indicator`); `3-led-direct-aspect` (3 Direct Lamp Control rows for R/G/Y LEDs, realises `signal-aspect` — aspect map: `stop` → red on + green off + yellow off; `approach` → yellow on; `clear` → green on); `2-led-bicolor-aspect` (2 Direct Lamp Control rows for R/G LEDs, also realises `signal-aspect` — aspect map: `stop` → red on + green off; `approach` → red on + green on, simulating yellow; `clear` → green on); `2-led-bicolor-flashing` (same 2 rows but configures blink mode — aspect map: `approach` → red blink + green blink for flashing yellow); a Mast-driven style would realise the same role through a single firmware mast resource that hides per-LED management entirely. The style's aspect-to-event map is what the template compiler uses at apply time: when a template rule says "set signal to approach," the compiler looks up the bound channel's style, reads its aspect map, and writes the concrete event IDs into the logic target's action slots. A role may be realised by one style or by several; the channel layer is permanently insulated from the difference.
...
|---|---|---|
| **Signal Aspect** | 1–3+ (depending on style: 2 for bicolor searchlight, 3 for tricolor, 1 for firmware mast) | Aspect value → style translates to the corresponding LED combination via its aspect-to-event map |
...
**Style polymorphism is hidden from the role.** A single role can be realised by different styles on different boards. A `signal-aspect` channel might use a Mast-driven style on Signal LCC (a single firmware-level mast that internally drives the LEDs by aspect rules), a `3-led-direct-aspect` style on a board exposing raw LED drivers, or a `2-led-bicolor-aspect` style on a board doing 2-LED color mixing. The facility slot doesn't know which — it sees only "a `signal-aspect` channel is bound here, and its style can render the aspects this template produces." For compound entities like a signal aspect, the user picks the style (e.g., `3-led-direct-aspect` vs `2-led-bicolor-aspect`) at planning time; the hardware-requirements report can then aggregate ("you need 3 more LED outputs") by inspecting bindings to placeholder-node pins. **Aspect compatibility is checked at bind time:** a template slot that produces `[stop, approach, clear]` accepts any channel whose style's aspect-to-event map covers those three aspects. A style that only supports `[stop, clear]` (2-aspect) would not be compatible.

**Multi-head masts.** At junctions, a single signal mast carries multiple heads — one per route. Each head is an independent `signal-aspect` channel, but the mast groups them into a facility-level unit for two reasons: (1) each head's rules depend on the turnout position (route interlocking), and (2) the upstream signal reads the mast's composite indication (most permissive aspect across all heads), not individual head states. See [Multi-Head Masts and Composite Indication](./app-ux-vision-feasibility.md#multi-head-masts-and-composite-indication) in the feasibility companion for the template DSL extension.
...
- **Plan-driven (guided):** A plan requirement says "needs `signal-aspect` channel (aspects: stop, approach, clear)" → user clicks "fulfill" → system shows compatible pins (filtered by available styles whose aspect-to-event maps cover the required aspects on connected hardware) → user picks → channel created and auto-linked to the plan. Best for: users following Plan → Wire.
...
- A **channel role** (e.g., `signal-aspect`, `block-occupancy`, `lamp-indicator`) is an abstract definition: what states does it have, what information does it carry, what templates can consume it. Roles are board-independent — `signal-aspect` means the same thing regardless of whether it's on a Signal LCC or a TowerLCC with signal driver. For roles with parameterized state vocabularies (like `signal-aspect`), the channel or template slot declares which specific states it supports or requires; compatibility is checked at bind time.
...
- A **board profile** declares which roles a board can host and which styles realise them, and maps each style to specific CDI fields. It says: "to create a `signal-aspect` channel on Signal LCC pin 3 via the `3-led-direct-aspect` style, set field X to value A, field Y to value B, and constrain field Z to options {C, D}; the style's aspect-to-event map declares how `stop` / `approach` / `clear` translate to specific lamp-row events." Different boards achieve the same role through different styles; different styles use different CDI field combinations and may support different aspect vocabularies.
