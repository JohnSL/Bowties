...
import { buildInteractionGroups, type Interaction, type InteractionType } from "./interaction-grouper.js";
...
// ─── Trace cache ─────────────────────────────────────────────────────────────
// The trace is parsed once on load and cached server-side. Analysis tools
// operate on the cache — no need to pass large trace text on every call.

interface TraceCache {
  frames: ReturnType<typeof parseTrace>["frames"];
  unparsedLines: ReturnType<typeof parseTrace>["unparsedLines"];
  aliasMap: ReturnType<typeof buildAliasMap>;
  groups: Interaction[];
}

let traceCache: TraceCache | null = null;

function loadAndCache(text: string): object {
  ...
  const hasBootFrames = frames.some(
    f => f.decoded.mtiValue === 0x19100 || f.decoded.mtiValue === 0x19101
  );
  const firstTs = frames[0]?.timestamp;
  const lastTs = frames[frames.length - 1]?.timestamp;

  const groupCounts: Record<string, number> = {};
  for (const g of groups) groupCounts[g.type] = (groupCounts[g.type] ?? 0) + 1;
...
server.tool(
  "read_trace_file",
  "Read a saved JMRI LCC Monitor trace from a file on disk. Returns the raw trace text.",
  { path: z.string().describe("Absolute path to the trace file") },
  async ({ path }) => {
...
server.tool(
  "read_clipboard",
  "Read the current clipboard contents. Use this when the user has copied a trace from JMRI but hasn't pasted it in the chat.",
  {},
  async () => {
...
server.tool(
  "load_trace",
  "Load a trace pasted directly into the chat. Parses and caches it server-side; analysis tools (summarize_session, list_groups, timeline) will use this cached trace automatically.",
  { trace: z.string().describe("Full text of the JMRI LCC Monitor trace") },
  ({ trace }) => {
...
server.tool(
  "parse_line",
  "Parse a single JMRI LCC Monitor trace line into its structured fields: timestamp, header, src/dest alias, MTI, data bytes, direction. Works on any single line with no prior context needed.",
  { line: z.string().describe("A single raw trace line from the JMRI LCC Monitor") },
  ({ line }) => {
...
server.tool(
  "decode_frame",
  "Decode a single JMRI trace line and return a human-readable one-line description of what the frame is and what it means. No prior context needed.",
  { line: z.string().describe("A single raw trace line from the JMRI LCC Monitor") },
  ({ line }) => {
    const frame = parseLine(line);
    if (!frame) {
    ...
    const extra: string[] = [];
    if (frame.srcName) extra.push(`Src name: ${frame.srcName}`);
    if (frame.destName) extra.push(`Dest name: ${frame.destName}`);
    if (frame.eventName) extra.push(`Event: ${frame.eventName}`);
...
server.tool(
  "summarize_session",
  "Summarise the currently loaded LCC trace: alias\u2192NodeID map, message counts per node, interaction group counts, and whether the trace starts from node boot. Call read_clipboard, read_trace_file, or load_trace first.",
  {},
  () => {
    if (!traceCache) {
    ...
    for (const frame of frames) {
      ...
      if (!perNode[aliasKey]) perNode[aliasKey] = {};
...
server.tool(
  "list_groups",
  "List interaction groups in the loaded trace. Requires at least one filter (type and/or nodeId). Returns protocol pairs (memory-config, snip, pip, verify, traction), events, alias negotiation, and individual frames. Each row includes groupIndex and frameIndices for drill-down via get_frames. Pre-computed timing fields (requestToAckMs, ackToReplyMs, replyToAckMs, gapToNextMs) enable latency analysis. Memory-config Read/Write requests are paired to their reply STRICTLY by payload address+space; `addressMatched: true` on a row means the pairing is trustworthy, `addressMatched: false` means no matching reply was found (rejected request, trace-window truncation, or dropped frames) and the request is emitted as unpaired.",
  {
    type: z.enum(INTERACTION_TYPES).optional().describe(
      "Filter by interaction type: memory-config | snip | pip | verify | traction | event | event-identification | alias-negotiation | datagram-ack | datagram | other"
    ),
    nodeId: z.string().optional().describe(
      "Filter to a specific node: NodeID like '09.00.99.05.01.C0' or alias like '0x646'"
    ),
    summaryOnly: z.boolean().optional().describe(
      "When true, omit the per-row `fields` object and the full `frameIndices` array. Keeps `groupIndex`, `src`, `dest`, `summary`, `complete`, `timing`, `frameCount`, and `addressMatched`. Use for large exploratory listings; drill down with a follow-up call (no summaryOnly) or with get_frames."
    ),
    limit: z.number().int().positive().optional().describe(
      "Maximum number of rows to return. When omitted, all matching rows are returned."
    ),
    offset: z.number().int().nonnegative().optional().describe(
      "Number of matching rows to skip before applying `limit`. Use with `limit` to page through large results."
    ),
  },
  ({ type, nodeId, summaryOnly, limit, offset }) => {
    ...
    let filtered = groups as Interaction[];
    if (type) filtered = filtered.filter(g => g.type === (type as InteractionType));
    ...
    const rows = paged.map(g => {
      ...
      if (g.destAlias !== undefined) row.dest = resolveAlias(g.destAlias, aliasMap);
      row.summary = g.summary;
      if (!g.complete) row.complete = false;
      if (!summaryOnly && Object.keys(g.fields).length > 0) row.fields = g.fields;
      if (g.addressMatched !== undefined) row.addressMatched = g.addressMatched;

      // Only include non-null timing values
      const t: Record<string, number> = {};
      for (const [k, v] of Object.entries(g.timing)) {
        if (v != null) t[k] = v as number;
      }
      if (Object.keys(t).length > 0) row.timing = t;

      row.frameCount = g.frameIndices.length;
      if (!summaryOnly) row.frameIndices = g.frameIndices;
      return row;
    });

    const payload: Record<string, unknown> = { groupCount: rows.length, groups: rows };
...
server.tool(
  "get_frames",
  "Get fully decoded details for specific frames by their indices (from list_groups or timeline). Use for drill-down after identifying interesting frames in a summary. Pass the frameIndices array from any list_groups result.",
  {
  ...
  ({ frameIndices }) => {
    ...
    const result = frameIndices
      ...
      .map(i => {
        ...
        if (f.jmriText) row.jmriText = f.jmriText;
        if (f.srcName)  row.srcName  = f.srcName;
        if (f.destName) row.destName = f.destName;
        if (f.eventName) row.eventName = f.eventName;
...
server.tool(
  "decode_datagram",
  "Decode a single datagram payload given as a hex string (space or no-space separated). Identifies the protocol (Memory Config, SNIP, PIP, Traction, etc.) and extracts fields. Fully context-free — no trace needed.",
  { bytesHex: z.string().describe("Hex bytes e.g. '20 41 00 00 02 64 08' or '204100000264 08'") },
  ({ bytesHex }) => {
...
server.tool(
  "timeline",
  "Show a chronological timeline of messages in the currently loaded trace, optionally filtered to a specific node (by NodeID or alias). Each entry shows the timestamp, node, message type, and destination. Call read_clipboard, read_trace_file, or load_trace first.",
  {
...
server.tool(
  "decode_diagnostic_frames",
  "Decode the recentFrameActivity array from a Bowties diagnostic report. Returns decoded frames grouped by protocol conversation (discovery, SNIP, datagram/memory-config, events). timestampMs counts DOWN from report capture time.",
  {
  ...
  ({ frames: diagFrames, knownNodes }) => {
    ...
    const aliasToNode: Map<number, { nodeId: string; snipName?: string }> = new Map();
    ...
    const conversations: { type: string; frames: unknown[] }[] = [];
    const discoveryFrames: unknown[] = [];
    const snipExchanges: unknown[] = [];
    const datagramFrames: unknown[] = [];
    const eventFrames: unknown[] = [];
    const otherFrames: unknown[] = [];

    for (const pf of parsed) {
      if (!pf || !pf.decoded) { otherFrames.push(pf); continue; }
      ...
      const resolveName = (alias: number | undefined) => {
        if (alias === undefined) return undefined;
        const entry = aliasToNode.get(alias);
        if (entry?.snipName) return `${entry.snipName} (0x${alias.toString(16).toUpperCase().padStart(3, "0")})`;
        if (entry?.nodeId) return `${entry.nodeId} (0x${alias.toString(16).toUpperCase().padStart(3, "0")})`;
    ...
    if (discoveryFrames.length) conversations.push({ type: "discovery", frames: discoveryFrames });
    if (snipExchanges.length) conversations.push({ type: "snip", frames: snipExchanges });
    if (datagramFrames.length) conversations.push({ type: "datagram/memory-config", frames: datagramFrames });
    if (eventFrames.length) conversations.push({ type: "events", frames: eventFrames });
    if (otherFrames.length) conversations.push({ type: "other", frames: otherFrames });
    ...
    const summary = {
      ...
      aliasMap: Object.fromEntries(
        [...aliasToNode.entries()].map(([alias, entry]) => [
...
server.tool(
  "analyze_diagnostic",
  "Analyze a complete Bowties diagnostic report JSON. Determines the session phase, interprets stats, decodes frame activity, flags anomalies, and suggests what additional data is needed.",
  {
  ...
  ({ report }) => {
    ...
    const log = (json.log ?? []) as string[];
    const recentFrames = (json.recentFrameActivity ?? []) as Array<{ direction: string; frame: string; timestampMs: number }>;
    const errors = ((stats.errors ?? []) as Array<Record<string, unknown>>);
    const cdiDownloads = (stats.cdiDownloads ?? stats.cdi_downloads ?? {}) as Record<string, unknown>;
    const configReads = (stats.configReads ?? stats.config_reads ?? {}) as Record<string, unknown>;
    const eventRoleExchange = stats.eventRoleExchange ?? stats.event_role_exchange ?? null;
    const discovery = (stats.discovery ?? {}) as Record<string, unknown>;
    const nodes = (discovery.nodes ?? []) as Array<Record<string, unknown>>;
    const probes = (discovery.probes ?? []) as Array<Record<string, unknown>>;
    ...
    let frameSummary: Record<string, unknown> = { totalFrames: recentFrames.length };
