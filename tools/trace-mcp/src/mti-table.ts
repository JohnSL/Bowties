...
export type MtiCategory =
  | "global"       // broadcast, no dest
  | "addressed"    // dest alias encoded in first 2 data bytes
  | "datagram"     // dest alias in header bits 23:12
  | "can-control"  // alias allocation frames (CID/RID/AMD/AME/AMR)
  | "event"        // carries an EventID payload
  | "unknown";
...
export function decodeHeader(header: number): DecodedHeader {
 ...
 */
export function extractAddrDest(data: number[]): { destAlias: number; payload: number[] } {
  if (data.length < 2) {
