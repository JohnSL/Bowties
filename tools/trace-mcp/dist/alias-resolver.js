...
function setIfBetter(map, alias, nodeId, confidence) {
 ...
 */
export function resolveAlias(alias, map) {
    const entry = map.get(alias);
    if (!entry)
        return `alias:0x${alias.toString(16).toUpperCase().padStart(3, "0")}`;
 ...
 */
export function aliasMapToObject(map) {
    const out = {};
