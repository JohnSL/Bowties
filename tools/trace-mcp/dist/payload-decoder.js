 ...
 */
const ADDRESS_SPACE_SUFFIX = { 0x01: 0xFD, 0x02: 0xFE, 0x03: 0xFF };
const MEM_CFG_COMMANDS = {
    0x40: "Read",
    0x41: "Read Reply (OK)",
...
function decodeMemoryConfig(bytes) {
    if (bytes.length < 2) {
        return {
    ...
    const fields = { command: `0x${cmd.toString(16).toUpperCase().padStart(2, "0")} (${cmdName})` };
    let summary = cmdName;
...
function addressSpaceName(space) {
    switch (space) {
...
function extractNullStrings(bytes) {
    const strings = [];
    let current = "";
 ...
 */
export function decodeDatagramPayload(bytes) {
    if (bytes.length === 0) {
...
//# sourceMappingURL=payload-decoder.js.map