...
function isMemConfigDatagram(dg) {
    return dg.bytes.length >= 2 && dg.bytes[0] === 0x20;
}
function isMemConfigReplyCmd(cmd) {
    return (cmd >= 0x48 && cmd <= 0x5F) ||
        cmd === 0x85 || cmd === 0x87 || cmd === 0x89 || cmd === 0x8B;
}
function isMemConfigNoReplyCmd(cmd) {
    // Fire-and-forget: only a transport ACK is expected, no reply datagram
    return cmd === 0xA8 || cmd === 0xAA || cmd === 0xDE;
}
function memCfgCmdName(cmd) {
    const base = cmd & 0xFC;
    const space = cmd & 0x03;
    const spaceSuffix = space === 0 ? "" : space === 1 ? " (0xFD)" : space === 2 ? " (0xFE)" : " (0xFF)";
    if (base === 0x40)
        return `Read${spaceSuffix}`;
    if (base === 0x44)
        return `Write${spaceSuffix}`;
    if (base === 0x48)
        return `Read Reply Fail${spaceSuffix}`;
    if (base === 0x50)
        return `Read Reply OK${spaceSuffix}`;
    if (base === 0x54)
        return `Write Reply OK${spaceSuffix}`;
    if (base === 0x58)
        return `Write Reply Fail${spaceSuffix}`;
    const EXACT = {
...
function eventIdFromBytes(bytes) {
 ...
 */
function addrFrameType(f) {
    if (!f.rawBytes.length)
        return "only";
...
// ─── Main Grouper ─────────────────────────────────────────────────────────────
export function buildInteractionGroups(frames, datagrams, _aliasMap) {
    const result = [];
    // Frame index sets for membership tracking
    const consumed = new Set(); // non-datagram frames assigned to an interaction
    const consumedDg = new Set(); // datagram indices already handled
    // Map every frame that belongs to a datagram
    const dgFrameSet = new Set();
    for (const dg of datagrams) {
        for (const fi of dg.frameIndices)
            dgFrameSet.add(fi);
    }
    /**
    ...
    for (const req of memReqs) {
        ...
        if (matchRep)
            frameIndices.push(...datagrams[matchRep.di].frameIndices);
        // Request ACK: Datagram Received OK from responder → requester
        const reqAckIdx = findFrame(reqLastFrameIdx, req.lastTs, matchRep?.firstTs ?? null, (f) => (f.decoded.mtiValue === MTI_DG_RECEIVED_OK || f.decoded.mtiValue === MTI_DG_REJECTED) &&
            f.decoded.srcAlias === dgReq.destAlias &&
            f.addrDestAlias === dgReq.srcAlias);
        if (reqAckIdx !== undefined) {
            frameIndices.push(reqAckIdx);
            consumed.add(reqAckIdx);
        }
        ...
        if (matchRep) {
            ...
            replyAckIdx = findFrame(repLastFrameIdx, matchRep.lastTs, null, (f) => (f.decoded.mtiValue === MTI_DG_RECEIVED_OK || f.decoded.mtiValue === MTI_DG_REJECTED) &&
                f.decoded.srcAlias === dgReq.srcAlias &&
                f.addrDestAlias === dgReq.destAlias);
            if (replyAckIdx !== undefined) {
                frameIndices.push(replyAckIdx);
                consumed.add(replyAckIdx);
            }
        }
        frameIndices.sort((a, b) => a - b);
        consumedDg.add(req.di);
        if (matchRep)
            consumedDg.add(matchRep.di);
        // Timing
        const timing = {};
        if (reqAckIdx !== undefined) {
        ...
        // Fields from request
        const fields = { command: memCfgCmdName(cmd) };
        if (dgReq.bytes.length >= 6) {
            ...
            if (dgReq.bytes.length > countOffset)
                fields.byteCount = dgReq.bytes[countOffset];
        }
        // Fields from reply
        if (matchRep) {
    ...
    const pendingSnipReqs = new Map(); // `${srcAlias}-${destAlias}` → frameIdx
    for (let i = 0; i < frames.length; i++) {
        ...
        if (f.decoded.mtiValue !== MTI_SNIP_RESPONSE)
            continue;
        if (f.addrDestAlias === undefined)
            continue;
        const ft = addrFrameType(f);
        if (ft !== "only" && ft !== "first")
            continue; // orphaned middle/last handled in step 12
        ...
        if (reqFrameIdx !== undefined)
            pendingSnipReqs.delete(reqKey);
        const frameIndices = [];
        if (reqFrameIdx !== undefined)
            frameIndices.push(reqFrameIdx);
        frameIndices.push(i);
        consumed.add(i);
        ...
        if (ft === "first") {
            ...
            for (let j = i + 1; j < frames.length; j++) {
                const nf = frames[j];
                if (consumed.has(j) || dgFrameSet.has(j))
                    continue;
                if (nf.decoded.mtiValue !== MTI_SNIP_RESPONSE)
                    break;
                if (nf.decoded.srcAlias !== responderAlias || nf.addrDestAlias !== requesterAlias)
                    break;
                frameIndices.push(j);
                consumed.add(j);
                if (addrFrameType(nf) === "last") {
                    complete = true;
                    i = j;
                    break;
                }
    ...
    const PAIR_SPECS = [
        ...
        },
        {
            ...
            buildFields: (_req, rep) => rep && rep.jmriNodeIds[0] ? { nodeId: rep.jmriNodeIds[0] } : {},
        },
        {
    ...
    for (const spec of PAIR_SPECS) {
        const pending = new Map(); // `${type}-${srcAlias}-${destAlias}` → frameIdx
        for (let i = 0; i < frames.length; i++) {
            const f = frames[i];
            if (consumed.has(i) || dgFrameSet.has(i))
                continue;
    ...
    // ── Step 4: Global Verify ──────────────────────────────────────────────────
    {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            if (consumed.has(i) || dgFrameSet.has(i))
                continue;
            if (f.decoded.mtiValue !== MTI_VERIFY_GLOBAL)
                continue;
            consumed.add(i);
        ...
        for (const reqIdx of pendingGlobal) {
            ...
            for (let i = reqIdx + 1; i < frames.length; i++) {
                ...
                if (mti !== MTI_VERIFIED_NODE && mti !== MTI_VERIFIED_SIMPLE)
                    continue;
                if (diffMs(reqTs, f.timestamp) > 500)
                    break;
                consumed.add(i);
                frameIndices.push(i);
                if (f.jmriNodeIds[0])
                    nodes.push(f.jmriNodeIds[0]);
            }
            result.push({
    ...
    // ── Step 5: Identify Events + identified responses ─────────────────────────
    {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            if (consumed.has(i) || dgFrameSet.has(i))
                continue;
            if (!IDENTIFY_MTIS.has(f.decoded.mtiValue))
                continue;
            consumed.add(i);
            ...
            for (let j = i + 1; j < frames.length; j++) {
                ...
                if (consumed.has(j) || dgFrameSet.has(j))
                    continue;
                if (!IDENTIFIED_MTIS.has(nf.decoded.mtiValue))
                    break;
                if (diffMs(f.timestamp, nf.timestamp) > 2000)
                    break;
                consumed.add(j);
                frameIndices.push(j);
                count++;
    ...
    // ── Step 6: Alias negotiation ─────────────────────────────────────────────
    {
        ...
        for (let i = 0; i < frames.length; i++) {
            ...
            if (f.decoded.isCidFrame) {
                ...
                if (!cidByAlias.has(alias))
                    cidByAlias.set(alias, []);
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        if (consumed.has(i) || dgFrameSet.has(i))
            continue;
        if (f.decoded.mtiValue !== MTI_INIT_COMPLETE && f.decoded.mtiValue !== MTI_INIT_SIMPLE)
            continue;
        consumed.add(i);
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        if (consumed.has(i) || dgFrameSet.has(i))
            continue;
        if (f.decoded.mtiValue !== MTI_PCER && f.decoded.mtiValue !== MTI_LEARN_EVENT)
            continue;
        consumed.add(i);
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        if (consumed.has(i) || dgFrameSet.has(i))
            continue;
        if (!IDENTIFIED_MTIS.has(f.decoded.mtiValue))
            continue;
        consumed.add(i);
        const eventId = eventIdFromBytes(f.rawBytes);
    ...
    for (let i = 0; i < frames.length; i++) {
        ...
        if (consumed.has(i) || dgFrameSet.has(i))
            continue;
        const mti = f.decoded.mtiValue;
        if (mti !== MTI_DG_RECEIVED_OK && mti !== MTI_DG_REJECTED)
            continue;
        consumed.add(i);
    ...
    for (const group of byKey.values()) {
        ...
        for (let i = 0; i < group.length - 1; i++) {
            ...
            if (lastFi !== undefined && firstFi !== undefined) {
                ...
                if (lastTs && firstTs)
                    curr.timing.gapToNextMs = diffMs(lastTs, firstTs);
            }
        }
    }
    // ── Step 14: Assign stable groupIndex ────────────────────────────────────
    for (let i = 0; i < result.length; i++)
        result[i].groupIndex = i;
    return result;
}
//# sourceMappingURL=interaction-grouper.js.map