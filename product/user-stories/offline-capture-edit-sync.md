...
- When an in-memory offline edit layers on top of an already-saved pending offline change, the transient edit is shown and reverted relative to the current pending value rather than the original bus baseline.
...
- Connector-generated follow-up edits are auto-cleared when the selection no longer requires them, unless the user has since replaced that auto-applied value with their own offline edit.
